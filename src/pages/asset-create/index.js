...
const Index = () => {
    ...
    const { values, setFieldValue, handleSubmit, handleReset, errors, touched, isValid } = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            let response = await dispatch(action.createAsset(formData));
    ...
    return load_location && load_work_unit && load_term && load_code && load_purchase_location ? (
        <Preloader />
    ) : (
        <div className='p-4'>
                    ...
                    <div>
                        <Form>
                            ...
                            >
                                ...
                                {all_purchase_location.map((code, index) => {
                                    return (
                                        <option key={index} value={code.id}>{code.name}</option>
                                    );
                                })}
