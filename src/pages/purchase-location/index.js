...
const Index = () => {
  ...
  const { load_purchase_location, purchase_location_with_pagination } = useSelector((state) => state.purchaseLocation);
  const { load_pagination, pagination } = useSelector((state) => state.pagination);
  const { open_confirm } = useSelector((state) => state.alert);

  const loadData = async () => {
    await dispatch(action.allPurchaseLocationWithPagination(page, limit, search));
  };
  ...
  const loadPurchaseLocation = async(page) => {
    setPage(page);
    await dispatch(action.allPurchaseLocationWithPagination(page, limit, search));
  }
  ...
  const { values, setFieldValue, handleSubmit, handleReset, errors, touched, isValid } = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      if(values.action == "update"){
          response = await dispatch(action.updatePurchaseLocation(values));
      }else{
          response = await dispatch(action.addPurchaseLocation(values));
      }
  ...
  const removeData = async () => {
    await dispatch(action.deletePurchaseLocation(id));
    loadData();
  };

  const handleSearch = async(keyword) => {
    setSearch(keyword);
    await dispatch(action.allPurchaseLocationWithPagination(1, limit, keyword));
  }

  const handleSelect = async(select) => {
    setLimit(select);
    await dispatch(action.allPurchaseLocationWithPagination(1, select, search));
  }

  return load_purchase_location && load_pagination ? (
    <Preloader />
  ) : (
    <div className="p-4">
      ...
      <Table striped bordered responsive>
        ...
        <tbody>
          {purchase_location_with_pagination.length > 0 ? (
            purchase_location_with_pagination.map((code, index) => {
              return (
                <tr key={index}>
                  <td>{index+1}</td>
                  <td>{code.name}</td>
      ...
      {purchase_location_with_pagination.length > 0 && pagination && 
        <ListGroup className='d-flex flex-row flex-wrap justify-content-center'>
          {listPage.map((list, index) => {
            return (
              <ListGroup.Item 
                key={index}
                onClick={() => loadPurchaseLocation(list.page)}
