...
const Index = () => {
    ...
    [totalPrice, setTotalPrice] = useState(location.state.total),
    ...
    const loadData = async() => {
        ...
        await dispatch(action.listAsset(detail_repair.type, filterLocation));
        ...
        onSubmit: async (values) => {
            if(values.action == "submit"){
                ...
                data.forEach(element => {
                    if(element.id_detail_repair){
                        formData.append("id_detail_repair[]", element.id_detail_repair);
                    }
                });

                idAsset.forEach(element => {
                    formData.append("id_asset[]", element);
                });

                quantity.forEach(element => {
                    formData.append("quantity[]", element);
                });
                ...
                let response = await dispatch(action.updateRepair(formData));
    ...
    useEffect(() => {
        if(Object.keys(detail_repair).length > 0){
            if(detail_repair.type != 1){
                setImagePreview(process.env.REACT_APP_BASE_URL + detail_repair.image_report.image_before);
            }
            setFieldValue('reason', detail_repair.reason);
        }
    }, [detail_repair]);
    ...
    const handleRemove = (index, item) => {
        ...
        let tmpArrIdRepair = [];
        for(let i = 0; i < idRepair.length; i++){
            if(i != index){
                tmpArrIdRepair.push(idRepair[i]);
            }
        }
        setPrice(tmpArrIdRepair);
    }
    ...
    return load_auth && load_asset && load_repair ? (
        <Preloader/>
    ) : (
        <div className='p-4'>
            <Button 
                            ...
                            <td colSpan={2}>
                                {detail_repair.type != 1 ? (
                                    <div className='d-flex'>
                                        <img src={imagePreview} alt="Photo Profile" className="mx-auto" onClick={handleImageRef} style={{cursor: 'pointer'}} width='50%' />
